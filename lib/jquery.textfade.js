...
(function() {
  ...
  TextFade = function($element, action, options) {
    var $, BLANK_CHARS_REGEX, LINES_SPLIT_REGEX, NOT_NEWLINE_REGEX, SEQUENCES, blankText, capitalize, defaultSettings, flatten, include, max, sequence, shuffle, skipChars, text, textToSequences, times, zip, _ref;
    ...
    sequence = $.type(this.settings.sequence) === 'string' ? SEQUENCES[this.settings.sequence](text) : $.isFunction(this.settings.sequence) ? this.settings.sequence(text) : this.settings.sequence.slice(0);
    skipChars = $.type(this.settings.skipChars) === 'regexp' ? (function(_this) {
      return function(c) {
        return _this.settings.skipChars.test(c);
      };
    })(this) : $.type(this.settings.skipChars) === 'string' ? (function(_this) {
      return function(c) {
        return include(_this.settings.skipChars.split(''), c);
      };
    })(this) : this.settings.skipChars;
    if ((skipChars != null) && skipChars !== false) {
      times(sequence.length, function(i) {
        if ((sequence[i] != null) && skipChars(text.charAt(sequence[i]))) {
          return sequence.splice(i, 1);
        }
      });
    }
